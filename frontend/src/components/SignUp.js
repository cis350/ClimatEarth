...
function Signup() {
  let user; // will store the username. this value is destroyed after each rendering
  ...
  const handleUsernameChange = (e) => {
    user = e.target.value; // update local variable
  ...
  const handleSignup = async (e) => {
    ...
    const success = await signupUser(user, password, name);
  ...
  return (
    ...
    <p>Already have an account? <a href={`/${username}/login`}>Login</a></p>
    </div>
    </div>
  );
}
